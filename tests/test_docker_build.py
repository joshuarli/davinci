"""Integration tests that build real KISS core packages inside Docker.
...
Usage:
  python3 -m pytest tests/test_docker_build.py -v --tb=short
...
TESTS = ROOT / "tests"
IMAGE_NAME = "pm-integration-test"
...
def setUpModule():
    ...
    r = subprocess.run(
        ["docker", "build", "-t", IMAGE_NAME, "-f", str(TESTS / "Dockerfile"), "."],
        cwd=str(ROOT),
        capture_output=True,
        text=True,
        timeout=300,
...
class DockerPMTestCase(unittest.TestCase):
    """Base class that runs tests inside a persistent Docker container.

    Each test class gets its own container with a fresh KISS_ROOT. The
    container persists across tests within the class so that packages
    built in earlier tests are available as dependencies for later ones.
    """
    ...
    @classmethod
    def setUpClass(cls):
        cls.container = f"pm-test-{cls.__name__.lower()}"
    ...
    def pm(self, *args, check=True):
        cmd = "KISS_ROOT=/kiss-root kiss " + " ".join(args)
...
class TestChecksumVerification(DockerPMTestCase):
    """Test that pm correctly verifies checksums for real packages."""
...
class TestBuildBaseSystem(DockerPMTestCase):
    ...
    def test_93_remove_and_reinstall(self):
        ...
        # pigz is a leaf (nothing depends on it).
