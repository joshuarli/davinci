...
core:
	docker build --build-context packages=$(PACKAGES_DIR) \
		--network=host \
		--build-arg REPO_URL=http://localhost:3000 \
		-t kominka:core .
...
# Build a package with Debian GCC. Usage: make rebuild-glibc-debian
# (must come before rebuild-% to take priority)
rebuild-%-debian:
...
# Build a package in kominka:core (zig cc). Usage: make rebuild-curl
rebuild-%:
	@test -f $(REPO_ENV) || { echo "error: $(REPO_ENV) not found"; exit 1; }
